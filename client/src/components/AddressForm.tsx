        ...
        <div onClick={resetForm} className="fixed inset-0 z-50 flex-center p-4">
            <form role="dialog" aria-modal="true" aria-labelledby="address-form-title" onKeyDown={event=>{if(event.key==="Escape") resetForm()}} onClick={event=>event.stopPropagation()} onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 w-full max-w-lg animate-fade-in">
                ...
                <div className="space-y-4">
                    <label className="block text-sm font-medium text-app-green">Label<input ref={firstInputRef} type="text" required value={form.label} onChange={event=>update({label:event.target.value})} placeholder="Home, Work, etc." className="mt-1.5 w-full px-4 py-2.5 text-sm rounded-xl border border-app-border focus:border-app-green outline-none" /></label>
                    ...
                    <div className="rounded-xl border border-app-border bg-app-cream/60 p-4" aria-live="polite"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium text-app-green">Current location</p><p className="mt-1 text-xs text-app-text-light">Collects multiple high-accuracy GPS readings and automatically fills the address.</p></div><button type="button" disabled={locating} onClick={useCurrentLocation} className="shrink-0 rounded-lg bg-app-orange px-3 py-2 text-xs font-semibold text-white disabled:opacity-60 flex items-center gap-1">{locating?<LoaderCircleIcon className="size-3.5 animate-spin"/>:<LocateFixedIcon className="size-3.5"/>}{locating?"Detecting...":"Detect location"}</button></div>{hasDetectedLocation&&<p className="mt-2 text-xs font-semibold text-green-700">Location detected within ±{form.mapLocationAccuracy} m and address autofilled.</p>}</div>
                </div>
