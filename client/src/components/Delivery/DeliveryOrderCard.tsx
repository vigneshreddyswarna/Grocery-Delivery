        ...
        <div key={order.id} className="bg-white rounded-2xl border border-app-border overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-app-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <span className="text-sm font-mono text-zinc-500">#{String(order.id || "").slice(-6).toUpperCase()}</span>
                    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${statusColors[order.status] || "bg-zinc-100 text-zinc-600"}`}>
                        {order.status}
                    </span>
                </div>
                <span className="text-sm font-semibold text-zinc-900">{currency}{(Number(order.total) || 0).toFixed(2)}</span>
            </div>
            ...
            <div className="px-5 py-4 space-y-3">
                ...
                <div className="flex items-start gap-2 text-sm text-zinc-600">
                    ...
                    <p>{order.shippingAddress?.address || "Address unavailable"}, {order.shippingAddress?.city || ""}, {order.shippingAddress?.state || ""} {order.shippingAddress?.zip || ""}</p>
                </div>
