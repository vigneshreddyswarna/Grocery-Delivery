    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex-center p-4">
                <div role="dialog" aria-modal="true" aria-labelledby="delivery-otp-title" className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
                    <h3 id="delivery-otp-title" className="text-lg font-semibold text-app-green mb-2">Enter Delivery OTP</h3>
                    <p className="text-sm text-zinc-500 mb-5">Ask the customer for the 6-digit OTP shown on their tracking page.</p>
                    <input aria-label="Delivery OTP" inputMode="numeric" autoFocus type="text" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))} placeholder="000000" className="w-full px-4 py-3 text-center text-2xl font-mono tracking-[0.5em] rounded-xl border border-app-border focus:border-app-green outline-none mb-4" />
