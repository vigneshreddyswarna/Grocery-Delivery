...
export const createOrder = async (req, res) => {
    ...
    const requestedItems = [...quantities].map(([product, quantity]) => ({ product, quantity }));
    ...
    if (!order)
        return res.status(409).json({ message: "An item became unavailable. Please review your cart." });
...
export const confirmOrderPayment = async (req, res) => {
    ...
    if (!sessionId)
        return res.status(400).json({ message: "Missing checkout session" });
    const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);
    const session = await stripe.checkout.sessions.retrieve(sessionId);
    const orderId = session.metadata?.orderId;
    if (!orderId)
        return res.status(400).json({ message: "Checkout session has no order" });
